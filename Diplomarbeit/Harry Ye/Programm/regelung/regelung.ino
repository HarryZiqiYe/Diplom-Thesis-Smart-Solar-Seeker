...
void loop() {
  ...
  */
  ...
  while(current_mA < 200 && current_position <= 360){
    current_mA = ina219.getCurrent_mA();
    Serial.print("Current[mA]: "); Serial.println(current_mA);
    ...
    if(oldcurrent_mA < current_mA){
      highestcurrent_mA = current_mA;
      highest_position = current_position;
    }
    if(oldcurrent_mA > (current_mA+tolerance1)){
      Serial.print("OLDCurrent[mA]: "); Serial.println(oldcurrent_mA);
      Serial.print("Current[mA]: "); Serial.println(current_mA);
      diff_position = current_position - highest_position;
      Serial.println("counterclockwise");
      myStepper.step(-(diff_position+20));
      current_position -= diff_position;
      Serial.println(current_position);
      linflag = 1;
      break;
    }
